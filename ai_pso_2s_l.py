...
N_atoms=10
dim = 3 * N_atoms
#of = lambda x: benchmark.lennard_jones_cluster(x, N_atoms)

def of_batch(X, N_atoms=N_atoms):
    X = np.asarray(X, dtype=float)

    if X.ndim == 1:
        return benchmark.lennard_jones_cluster(X, N_atoms)

    # X is (n, D): evaluate each row
    return np.array([benchmark.lennard_jones_cluster(x, N_atoms) for x in X], dtype=np.float32)
...
for j in range(0 ,nb_runs):
    ...
    for i in range(1, max_zone +1):
        ...
        model, scalers, device = train_surrogate(model=model ,  D=dim, n=200, epochs=30, bounds=(zone_min, zone_max) , function=of)
        best_x, best_yhat = minimize_surrogate_gd(model, scalers, D=dim, bounds=(zone_min,zone_max), steps=150, lr=0.05, n_restarts=2, device=device)
        particles_valid.append(best_x)
        

    particles = np.array(particles_valid)
    ...
    if best_score < min_err :
        min_err = best_score
    if np.abs(best_score - real_best ) < 0.0001:
        sr+=1 
